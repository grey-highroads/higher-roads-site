...
// v8.0 user-authored reference guidance: the universal prompt contract accepts a
// universal creative dossier and optional locked_assets while preserving the
...
(function(){
  window.HR_CREATIVE_CONFIG = {
    config_version: 'creative-config-2026-07-25-user-reference-guidance-v8',
    version: 'creative-config-2026-07-25-user-reference-guidance-v8',
...
MULTI-ANCHOR COMPOSITION. When the dossier contains multiple anchors, preserve their provenance and use their declared roles as contribution domains rather than numeric weights. A visual_system anchor principally informs palette, materials, design language, composition rhythm, and recurring visual relationships. An emotional_cinematic_reference principally informs emotional register, gesture, light behavior, pacing, and temporal or spatial grammar. These are priorities, not prohibitions. Each thesis must name at least one concrete contribution from every supplied reference anchor in belongs_because and source_evidence unless explicit intent, a user-authored constraint, or delivery context makes that evidence unusable. When evidence disagrees, resolve it in favor of explicit intent and delivery context; never average the anchors into generic mood language and never silently drop one.
...
MULTI-ANCHOR RESOLUTION. When multiple anchors are supplied, the winner must retain a traceable contribution from each role without becoming a collage. Compare how each candidate resolves agreement or conflict between anchors. A visual_system anchor principally governs repeated visual language; an emotional_cinematic_reference principally governs emotional, lighting, gesture, pacing, and temporal behavior. These role boundaries are priorities, not automatic exclusions. Disqualify any candidate that silently drops an anchor, copies one literally, or blends both into generic adjectives. State the winning resolution in selection_reason and name any anchor evidence rejected because it conflicts with intent, user-authored constraints, or delivery context.
...
MULTI-ANCHOR PROVENANCE. When more than one anchor is present, the scene must visibly carry at least one concrete contribution from each anchor role while remaining one coherent world. Let visual_system evidence principally guide palette, materials, recurring relationships, and composition rhythm. Let emotional_cinematic_reference evidence principally guide emotional register, gesture, light, pacing, and temporal or spatial grammar. These are contribution priorities rather than prohibitions. Name the role-specific contributions in creative_rationale and evidence_cues. If intent or delivery context forces a conflict resolution, explain the resolution instead of silently dropping or averaging an anchor.
